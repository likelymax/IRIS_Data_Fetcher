classdef irisFetch
   % This is the revised irisFetch by Yitan Wang to fit the download code
   % automatically. Once you want to use it, you have to change the name of
   % the file to "irisFetch.m" again.
   % this is the copy to save the revised version
   %
...
%                          tracedata.setSACPZ_URL(url)
      ...
      function [events, urlParams, st] = Events(varargin)
   ...
   methods(Static, Access=protected)
      ...
      function mts = convertTraces(traces)
         ...
         blankTrace = struct('network','','station','','location','',...
            ...
            'eventlatitude',0,'eventlongitude',0,'eventdepth',0,'eventmag',0,...
      ...
      function [ tr_out ] = read_sac_in(fileDirectory)
         ...
         for i=1:numel(fileNames)
             ...
             if sac.LEVEN~=1
                ...
                tr_out(i).eventlongitude = sac.EVLO;
                tr_out(i).eventlatitude = sac.EVLA;
                tr_out(i).eventdepth    = sac.EVDP;
                tr_out(i).eventmag      = sac.MAG;
      ...
      function [ output_args ] = write_sac_out(writeDirectory, traces, verbosity)
         ...
         function write_trace_header_to_sac(fid, javaTrace)
            ...
            if isjava(javaTrace)
               ...
               endTime = javaTrace.getEndTime();
               ev = irisFetch.Events('starttime',startTime,'endtime',endTime,'minmag', 6.0, 'limit', 1 ,'includeallorigins',true,'includeallmagnitudes',true);
               write_sac_field( 'KENVM', ev.PreferredTime);
               write_sac_field( 'EVLO', ev.PreferredLongitude);
               write_sac_field( 'EVLA', ev.PreferredLatitude);
               write_sac_field( 'MAG', ev.Magnitudes.Value);
               write_sac_field( 'EVDP', ev.PreferredDepth);

               write_sac_field(fid, 'LEVEN', true); % evenly spaced data
               write_sac_field(fid, 'LCALDA', true);
               write_sac_field(fid, 'LOVROK', true);
            ...
            elseif isstruct(javaTrace)
               ...
               startTime = javaTrace.startTime;
               endTime = javaTrace.endTime;
      ...
      %         write_sac_field(fid, 'KENVM', eventID); 
...
%                 disp(ev.PreferredTime);
               ...
               write_sac_field(fid, 'LCALDA', true);
               write_sac_field(fid, 'LOVROK', true);
      %         write_sac_field(fid, 'OMARKER', -600.);
               write_sac_field(fid, 'DELTA', 1/javaTrace.sampleRate); % ODELTA would have observed change...
               ev = irisFetch.Events('starttime',startTime,'endtime',endTime,'minmag', 6.0, 'limit', 1 ,'includeallorigins',true,'includeallmagnitudes',true);
               if isempty(ev) == 1
                   
                  disp('No event was found');
               
               else
                   
              % ev = irisFetch.Events('starttime',startTime,'endtime',endTime,'minmag', 6.0, 'limit', 1 ,'includeallorigins',true,'includeallmagnitudes',true);
               
               write_sac_field(fid, 'EVLO', ev.PreferredLongitude);
               write_sac_field(fid, 'EVLA', ev.PreferredLatitude);
               write_sac_field(fid, 'MAG', ev.Magnitudes.Value);
               write_sac_field(fid, 'EVDP', ev.PreferredDepth);
